...
<head>
    ...
    <title>用户中心-订单列表|大剧院_影剧在线选座购票平台</title>
...
$(".order-datail").overlay({
    ...
    onBeforeLoad: function (t) {
        ...
        html += '<ul><li>剧院：' + data.attr("data-cinemaname") + '</li>';
        html += '<li>场次：' + data.attr("data-showtime") + '</li>';
        html += '<li>剧厅：' + data.attr("data-hallname") + '</li>';
        ...
        if (data.attr("data-status") == "1") {
            ...
            if (data.attr("data-HaveATM")=="true") {
                html += '<p class="cor999">请凭取票码至剧院自动取票机或前台售票窗口取票</p>';
            } else {
                html += '<p class="cor999">请凭取票码至剧院前台售票窗口取票</p>';
            }
