    ...
    <main class="lyear-layout-content">
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header"><h4>编辑影剧院</h4></div>
              <div class="card-body">
                <form action="add" id="cinema-add-form" method="post" class="row">
                  ...
                  <div class="form-group col-md-12">
                    ...
                    <div class="form-controls">
                      <ul class="list-inline clearfix lyear-uploads-pic" id="show-uploaded-pic">
                        ...
                        <img src="/photo/view?filename=${cinema.picture}">
                        <#else>
                        ...
                        <input type="hidden" name="picture" id="picture" value="${cinema.picture}">
                  ...
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">影剧院名称</span>
                    <input type="text" class="form-control required" id="name" name="name" value="${cinema.name}" placeholder="请输入影剧院名" tips="请填写影剧院名" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">影剧院地区</span>
                    <select class="form-control" id="cinema-area-province" style="width:33%;">
                    	<option value="">请选择省份</option>
                    	<#list provinceList as province>
                    	<option value="${province.id}" <#if cinema.area.provinceId == province.id>selected</#if>>${province.name}</option>
                    	</#list>
                    </select>
                    <select class="form-control" id="cinema-area-city" style="width:33%;">
                    	<#list cityList as city>
                    	<option value="${city.id}" <#if cinema.area.cityId == city.id>selected</#if>>${city.name}</option>
                    	</#list>
                    </select>
                    <select name="area" class="form-control" id="cinema-area-district" style="width:33%;">
                    	<#list districtList as district>
                    	<option value="${district.id}" <#if cinema.area.id == district.id>selected</#if>>${district.name}</option>
                    	</#list>
                    </select>
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">剧院服务</span>
                    ...
                    <input type="hidden" name="cinemaService" id="cinema-service" value="${cinema.cinemaService}" class="required" tips="请选择影剧院服务">
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">剧院地址</span>
                    <input type="text" class="form-control required" id="address" name="address" value="${cinema.address!""}" placeholder="请填写影剧院详细地址" tips="请填写影剧院详细地址" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">联系电话</span>
                    <input type="text" class="form-control required" id="tel" name="tel" value="${cinema.tel!""}" placeholder="请填写影剧院联系电话" tips="请填写影剧院联系电话" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">交通路线</span>
                    <input type="text" class="form-control required" id="transport" name="transport" value="${cinema.transport!""}" placeholder="请填写影剧院交通路线" tips="请填写影剧院交通路线" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">剧院介绍</span>
                    <textarea class="form-control" id="info" name="info" rows="5" placeholder="请填写影剧院介绍">${cinema.info!""}</textarea>
                  </div>
...
$(document).ready(function(){
	...
	$("#add-form-submit-btn").click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					showSuccessMsg('影剧院添加成功!',function(){
	...
	//勾选已选中的剧院服务
	$("input.cinema-service[type='checkbox']").each(function(i,e){
		if($("#cinema-service").val().indexOf($(e).val()) >= 0){
			$(e).prop('checked',true);
		}
	});
});
