    ...
    <main class="lyear-layout-content">
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header"><h4>编辑剧厅</h4></div>
              <div class="card-body">
                <form action="add" id="cinema-hall-add-form" method="post" class="row">
                  <input type="hidden" name="id" value="${cinemaHall.id}">
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">剧厅名称</span>
                    <input type="text" class="form-control required" id="name" name="name" value="${cinemaHall.name}" placeholder="请输入剧厅名" tips="请填写剧厅名" />
                  </div>
                  <div class="input-group m-b-10">
                    ...
                    <select class="form-control" name="cinema" id="cinema-area-province">
                    	...
                    	<option value="${cinema.id}" <#if cinema.id == cinemaHall.cinema.id>selected</#if>>${cinema.name}</option>
                    	</#list>
                    </select>
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">横向座位数</span>
                    <input type="number" class="form-control required" disabled id="max-x" name="maxX" value="${cinemaHall.maxX}" placeholder="请填写剧厅最大横向座位数" tips="请填写剧厅最大横向座位数" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">纵向座位数</span>
                    <input type="number" class="form-control required" disabled id="max-y" name="maxY" value="${cinemaHall.maxY}" placeholder="请填写剧厅最大纵向座位数" tips="请填写剧厅最大纵向座位数" />
                  </div>
	...
	$("#add-form-submit-btn").click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					showSuccessMsg('剧厅编辑成功!',function(){
