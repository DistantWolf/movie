    ...
    <main class="lyear-layout-content">
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header"><h4>编辑排剧场次</h4></div>
              <div class="card-body">
                <form action="add" id="cinema-hall-add-form" method="post" class="row">
                  <input type="hidden" name="id" value="${cinemaHallSession.id}">
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">影剧名称</span>
                    <select class="form-control" name="movie" >
                    	<#list movieList as movie>
                    	<option value="${movie.id}" <#if movie.id == cinemaHallSession.movie.id>selected</#if>>${movie.name}</option>
                    	</#list>
                    </select>
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">所属剧院</span>
                    <select class="form-control" name="cinema" id="cinema">
                    	<option value="">请选择所属剧院</option>
                    	<#list cinemaList as cinema>
                    	<option value="${cinema.id}" <#if cinema.id == cinemaHallSession.cinema.id>selected</#if>>${cinema.name}</option>
                    	</#list>
                    </select>
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">所属剧厅</span>
                    <select class="form-control" name="cinemaHall" id="cinema-hall">
                    	<option value="${cinemaHallSession.cinemaHall.id}">${cinemaHallSession.cinemaHall.name}</option>
                    </select>
                  </div>
                  <div class="input-group m-b-10">
                    ...
                    <select class="form-control" name="cinemaSessionType" id="cinemaSessionType">
                    	...
                    	<option value="${cinemaSessionType}" <#if cinemaSessionType == cinemaHallSession.cinemaSessionType>selected</#if>>${cinemaSessionType.getName()}</option>
                    	</#list>
                    </select>
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">上映日期</span>
                    <input type="text" class="form-control required js-datetimepicker" id="show-date" name="showDate" value="${cinemaHallSession.showDate}" placeholder="请选择场次上映日期" tips="请选择场次上映日期" data-side-by-side="true" data-locale="zh-cn" data-format="YYYY-MM-DD" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">上映时间</span>
                    <input type="text" class="form-control required js-datetimepicker" id="show-time" name="showTime" value="${cinemaHallSession.showTime}" placeholder="请选择场次上映时间" tips="请选择场次上映时间" data-side-by-side="true" data-locale="zh-cn" data-format="HH:mm" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">剧票原价</span>
                    <input type="number" class="form-control required" id="old-price" name="oldPrice" value="${cinemaHallSession.oldPrice!"0"}" placeholder="请填写剧票原价" tips="请填写剧票原价" />
                  </div>
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">剧票现价</span>
                    <input type="number" class="form-control required" id="new-price" name="newPrice" value="${cinemaHallSession.newPrice!"0"}" placeholder="请填写剧票现价" tips="请填写剧票现价" />
                  </div>
...
$(document).ready(function(){
	...
	$("#add-form-submit-btn").click(function(){
		...
		if($("#cinema").val() == ''){
			showErrorMsg('请选择所属剧院！');
			return;
		}
		if($("#cinema-hall").val() == ''){
			showErrorMsg('请选择所属剧厅！');
			return;
		}
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					showSuccessMsg('排剧场次添加成功!',function(){
	...
	$("#cinema").change(function(){
		...
		ajaxRequest('/admin/cinema_hall/get_cinema_halls','POST',{cid:cid},function(data){
			var halls = data.data;
			var options = '<option value="">请选择剧厅</option>';
